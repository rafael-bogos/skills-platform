...
export function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.error) {
        setError(
          result.error.message ?? 'Erro ao criar conta. Verifique se o email já está em uso.',
        )
      } else {
        router.push('/')
        router.refresh()
  ...
  async function handleSocial(provider: 'google' | 'github') {
    ...
    try {
      ...
      if (result?.error) {
        setError(result.error.message ?? `Erro ao entrar com ${provider}.`)
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-4 flex flex-col gap-2">
            <button
              ...
              className={cn(
                'flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium',
                'text-slate-700 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60',
          ...
          <div className="relative mb-4">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-2 text-xs text-slate-400 dark:bg-slate-900 dark:text-slate-500">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            <Button type="submit" size="md" disabled={loading} className="mt-1 w-full justify-center">
